...
class ModalQuitarParcela extends TWindow
{
    ...
    public function onPagar($param = null) 
    {
        try
        {
            ...
            if($lancamento->conta->tipo_conta_id == TipoConta::RECEBER){
                ...
                $extrato->entrada_valor = $object->valor;
            }else 
            if($lancamento->conta->tipo_conta_id == TipoConta::PAGAR){
                $extrato->tipo_extrato_id = TipoExtrato::PAGAR;
                $extrato->saida_valor = $object->valor;
            }
            ...
            if($object->compensado=='S'){
                ...
                $extrato->data_compensacao = $object->dt_pagamento;

                $extrato->ano = date('Y', strtotime($object->dt_pagamento));
                $extrato->mes = date('m', strtotime($object->dt_pagamento));
                $extrato->ano_mes = date('Ym', strtotime($object->dt_pagamento));
            ...
            $proximoLancamento = Lancamento::where('conta_id','=',$lancamento->conta->id)->where('parcela','=',((int)$lancamento->parcela +1))->load();
    ...
    {               
        ...
        {   
            ...
            $object = new stdClass();
            $object->valor = number_format($lancamento->valor, 2, ',', '.');
