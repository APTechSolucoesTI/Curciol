...
class ExtratoList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $tipo_extrato_id = new TDBCombo('tipo_extrato_id', 'escritorio', 'TipoExtrato', 'id', '{nome}','id asc' , $criteria_tipo_extrato_id );
        ...
        $data_lancamento_inicio = new TDate('data_lancamento_inicio');
        $ate_lancamento = new TLabel("até", null, '14px', null);
        $data_lancamento_fim = new TDate('data_lancamento_fim');
        ...
        $lancamento_tipo_pagamento_nome = new TDBCombo('lancamento_tipo_pagamento_nome', 'escritorio', 'TipoPagamento', 'id', '{nome}','nome asc' , $criteria_lancamento_tipo_pagamento_nome );
        $tipo_extrato_nome = new TDBCombo('tipo_extrato_nome', 'escritorio', 'TipoExtrato', 'id', '{nome}','nome asc' , $criteria_tipo_extrato_nome );
        ...
        $tipo_extrato_id->enableSearch();
        $tipo_extrato_nome->enableSearch();
        $lancamento_tipo_pagamento_nome->enableSearch();

        $data_lancamento_fim->setMask('dd/mm/yyyy');
        $data_compensacao_fim->setMask('dd/mm/yyyy');
        $prev_compensacao_fim->setMask('dd/mm/yyyy');
        $data_lancamento_inicio->setMask('dd/mm/yyyy');
        $data_compensacao_inicio->setMask('dd/mm/yyyy');
        $prev_compensacao_inicio->setMask('dd/mm/yyyy');

        $data_lancamento_fim->setDatabaseMask('yyyy-mm-dd');
        $data_compensacao_fim->setDatabaseMask('yyyy-mm-dd');
        $prev_compensacao_fim->setDatabaseMask('yyyy-mm-dd');
        $data_lancamento_inicio->setDatabaseMask('yyyy-mm-dd');
        ...
        $data_lancamento_fim->setSize('calc(50% - 30px)');
        $data_lancamento_inicio->setSize('calc(50% - 30px)');
        ...
        $row2 = $this->form->addFields([new TLabel("Compensado:", null, '12px', null, '100%'),$compensado],[new TLabel("Data da compensação:", null, '12px', null, '100%'),new TLabel("Previsão de compensação:", null, '12px', null, '100%'),$data_compensacao_inicio,$ate_compensacao,$data_compensacao_fim,$prev_compensacao_inicio,$ate_previsao,$prev_compensacao_fim]);
        $row2->layout = ['col-sm-4',' col-sm-8'];

        $row3 = $this->form->addFields([new TLabel("Data do lançamento:", null, '12px', null, '100%'),$data_lancamento_inicio,$ate_lancamento,$data_lancamento_fim]);
        $row3->layout = [' col-sm-4'];

        $row4 = $this->form->addFields([new TLabel("Histórico:", null, '12px', null, '100%'),$historico],[new TLabel("Valor da entrada (R$):", null, '12px', null, '100%'),$entrada_valor],[new TLabel("Valor da saída (R$):", null, '12px', null, '100%'),$saida_valor]);
        ...
        if(!empty($param['conta_caixa_id'] ?? null))
        {
            TSession::setValue(__CLASS__.'load_filter_conta_caixa_id', $param['conta_caixa_id'] ?? null);
        }
        $filterVar = TSession::getValue(__CLASS__.'load_filter_conta_caixa_id');
        $this->filter_criteria->add(new TFilter('conta_caixa_id', '=', $filterVar));
        ...
        $column_data_lancamento_transformed = new TDataGridColumn('data_lancamento', "Data do lançamento", 'center' , '10%');
        ...
        $column_data_lancamento_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            if(!empty(trim((string) $value)))
            {
                try
        ...
        $order_data_lancamento_transformed = new TAction(array($this, 'onReload'));
        $order_data_lancamento_transformed->setParameter('order', 'data_lancamento');
        $column_data_lancamento_transformed->setAction($order_data_lancamento_transformed);
        ...
        $this->datagrid->addColumn($column_data_lancamento_transformed);
        ...
        $panel = new TPanelGroup("Listagem de extratos - {$param['conta_caixa']}");
        ...
        TScript::create("$('label:contains(\"Previsão de compensação:\")').show();");
        TScript::create("$(\"[name='ate_previsao']\").closest('.fb-inline-field-container').show()");
        TScript::create("$(\"[name='prev_compensacao_inicio']\").closest('.fb-inline-field-container').show()");
        TScript::create("$(\"[name='prev_compensacao_fim']\").closest('.fb-inline-field-container').show()");

        TScript::create("$('label:contains(\"Data da compensação:\")').hide();");
        TScript::create("$(\"[name='ate_compensacao']\").closest('.fb-inline-field-container').hide()");
        TScript::create("$(\"[name='data_compensacao_inicio']\").closest('.fb-inline-field-container').hide()");
        TScript::create("$(\"[name='data_compensacao_fim']\").closest('.fb-inline-field-container').hide()");
    ...
    public static function onTrocaCompensado($param = null) 
    {
        try 
        {
            if(isset($param['compensado'])){
                if($param['compensado']=='S'){
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->conta_caixa_id) AND ( (is_scalar($data->conta_caixa_id) AND $data->conta_caixa_id !== '') OR (is_array($data->conta_caixa_id) AND (!empty($data->conta_caixa_id)) )) )
        {
            ...
            $filters[] = new TFilter('conta_caixa_id', '=', $data->conta_caixa_id);// create the filter 
        }
        ...
        if (isset($data->tipo_extrato_id) AND ( (is_scalar($data->tipo_extrato_id) AND $data->tipo_extrato_id !== '') OR (is_array($data->tipo_extrato_id) AND (!empty($data->tipo_extrato_id)) )) )
        {
            ...
            $filters[] = new TFilter('tipo_extrato_id', '=', $data->tipo_extrato_id);// create the filter 
        }
        ...
        if (isset($data->data_lancamento_inicio) AND ( (is_scalar($data->data_lancamento_inicio) AND $data->data_lancamento_inicio !== '') OR (is_array($data->data_lancamento_inicio) AND (!empty($data->data_lancamento_inicio)) )) )
        {

            $filters[] = new TFilter('data_lancamento', '>=', $data->data_lancamento_inicio);// create the filter 
        }

        if (isset($data->data_lancamento_fim) AND ( (is_scalar($data->data_lancamento_fim) AND $data->data_lancamento_fim !== '') OR (is_array($data->data_lancamento_fim) AND (!empty($data->data_lancamento_fim)) )) )
        {

            $filters[] = new TFilter('data_lancamento', '<=', $data->data_lancamento_fim);// create the filter 
        }
        ...
        if (isset($data->tipo_extrato_nome) AND ( (is_scalar($data->tipo_extrato_nome) AND $data->tipo_extrato_nome !== '') OR (is_array($data->tipo_extrato_nome) AND (!empty($data->tipo_extrato_nome)) )) )
        {
            ...
            $filters[] = new TFilter('tipo_extrato_id', '=', $data->tipo_extrato_nome);// create the filter 
        }
