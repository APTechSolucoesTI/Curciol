...
class AgendaList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $escritorio_nome = new TEntry('escritorio_nome');

        $nome_col->exitOnEnter();
        $escritorio_nome->exitOnEnter();

        $nome_col->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1']));
        $escritorio_nome->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1']));
        ...
        $escritorio_nome->setSize('100%');

        $row1 = $this->form->addFields([new TLabel("Escritório:", null, '14px', null, '100%'),$escritorio_id],[new TLabel("Nome:", null, '14px', null, '100%'),$nome]);
        ...
        $this->filter_criteria->add(new TFilter('escritorio_id', 'in', "(SELECT id FROM escritorio WHERE system_unit_id in ({$filterVar}))"));

        $this->datagrid->style = 'width: 100%';
        $this->datagrid->setHeight(250);

        $column_nome = new TDataGridColumn('nome', "Nome", 'left');
        $column_escritorio_nome = new TDataGridColumn('escritorio->nome', "Escritório", 'left' , '250px');
        ...
        $order_horario_inicial = new TAction(array($this, 'onReload'));
        $order_horario_inicial->setParameter('order', 'horario_inicial');
        $column_horario_inicial->setAction($order_horario_inicial);
        $order_horario_final = new TAction(array($this, 'onReload'));
        $order_horario_final->setParameter('order', 'horario_final');
        $column_horario_final->setAction($order_horario_final);
        $order_duracao = new TAction(array($this, 'onReload'));
        $order_duracao->setParameter('order', 'duracao');
        $column_duracao->setAction($order_duracao);

        $this->datagrid->addColumn($column_nome);
        $this->datagrid->addColumn($column_escritorio_nome);
        ...
        $tr->add($td_nome_col);
        $td_escritorio_nome = TElement::tag('td', $escritorio_nome);
        $tr->add($td_escritorio_nome);
        ...
        $this->datagrid_form->addField($escritorio_nome);
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['AgendaList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:table #00b894' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['AgendaList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['AgendaList', 'onExportXls']), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Configurações","Agendas"]));
        }
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->escritorio_nome) AND ( (is_scalar($data->escritorio_nome) AND $data->escritorio_nome !== '') OR (is_array($data->escritorio_nome) AND (!empty($data->escritorio_nome)) )) )
        {

            $filters[] = new TFilter('escritorio_id', 'in', "(SELECT id FROM escritorio WHERE nome like '%{$data->escritorio_nome}%')");// create the filter 
        }
