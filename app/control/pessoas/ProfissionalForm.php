...
class ProfissionalForm extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $criteria_tipo_profissional_id = new TCriteria();
        ...
        $bntBuscarCNPJ = new TButton('bntBuscarCNPJ');
        $nome = new TEntry('nome');
        $btnVerificarNome = new TButton('btnVerificarNome');
        $tipo_profissional_id = new TDBCombo('tipo_profissional_id', 'escritorio', 'TipoProfissional', 'id', '{nome}','nome asc' , $criteria_tipo_profissional_id );
        $registro_profissional = new TNumeric('registro_profissional', '0', ',', '.' );
        $orgao_registro_profissional = new TEntry('orgao_registro_profissional');
        $assinatura = new TFile('assinatura');
        $telefone = new TEntry('telefone');
        $email = new TEntry('email');
        $tratamento = new TDBEntry('tratamento', 'escritorio', 'Pessoa', 'tratamento','tratamento asc' , $criteria_tratamento );
        ...
        $pessoa_especialidade_pessoa_especialidade_id = new TDBCombo('pessoa_especialidade_pessoa_especialidade_id[]', 'escritorio', 'Especialidade', 'id', '{descricao}','descricao asc' , $criteria_pessoa_especialidade_pessoa_especialidade_id );
        $this->fieldList_62d07079690f0 = new TFieldList();
        $system_users_id = new TDBCombo('system_users_id', 'escritorio', 'SystemUsers', 'id', '{name}','name asc' , $criteria_system_users_id );
        ...
        $chave_aasp = new TEntry('chave_aasp');
        ...
        $tipo_pessoa_id->setChangeAction(new TAction([$this,'onLoadDocumentos']));

        $tipo_profissional_id->addValidation("Tipo de profissional", new TRequiredValidator()); 
        ...
        $registro_profissional->setAllowNegative(false);
        $orgao_registro_profissional->setTip("Exemplo: OAB/SP");
        ...
        $btnVerificarNome->setAction(new TAction([$this, 'onChangeNome']), "");
        ...
        $bntBuscarCNPJ->setAction(new TAction(['ModalBuscarCNPJ', 'onShow'],['campo' => '["nome", "telefone" ,"cpf_cnpj", "pessoa_endereco_pessoa_cep" ,"pessoa_endereco_pessoa_cidade_id", "pessoa_endereco_pessoa_id", "pessoa_endereco_pessoa_bairro", "pessoa_endereco_pessoa_rua" ,"pessoa_endereco_pessoa_numero" ,"pessoa_endereco_pessoa_complemento"]',"form" => self::$formName]), "Buscar");

        $bntBuscarCNPJ->addStyleClass('btn-default');
        ...
        $bntBuscarCNPJ->setImage('fas:search #000000');
        ...
        $tipo_profissional_id->enableSearch();
        $pessoa_especialidade_pessoa_especialidade_id->enableSearch();
        ...
        $tipo_profissional_id->setSize('100%');
        $registro_profissional->setSize('100%');
        ...
        $tipo_pessoa_id->setSize('calc(73% - 100px)');
        $orgao_registro_profissional->setSize('100%');
        ...
        $bcontainer_654e607d81be4 = new BootstrapFormBuilder('bcontainer_654e607d81be4');
        $this->bcontainer_654e607d81be4 = $bcontainer_654e607d81be4;
        $bcontainer_654e607d81be4->setProperty('style', 'border:none; box-shadow:none;');
        $row2 = $bcontainer_654e607d81be4->addFields([new TLabel("Tipo de pessoa:", '#FF0000', '12px', null, '100%'),$tipo_pessoa_id,$bntBuscarCNPJ],[new TLabel("Nome:", '#FF0000', '12px', null, '100%'),$nome,$btnVerificarNome]);
        $row2->layout = [' col-sm-6',' col-sm-6'];

        $row3 = $bcontainer_654e607d81be4->addFields([new TLabel("Tipo de profissional:", '#FF0000', '12px', null, '100%'),$tipo_profissional_id],[new TLabel("Registro profissional:", '#000000', '12px', null, '100%'),$registro_profissional],[new TLabel("Orgão emissor:", '#000000', '12px', null, '100%'),$orgao_registro_profissional]);
        $row3->layout = ['col-sm-4','col-sm-4','col-sm-4'];

        $row4 = $bcontainer_654e607d81be4->addFields([new TLabel("Assinatura para documentos:", null, '12px', null),$assinatura],[new TLabel("Telefone:", null, '12px', null, '100%'),$telefone],[new TLabel("Email:", null, '12px', null, '100%'),$email],[new TLabel("Tratamento:", null, '12px', null, '100%'),$tratamento]);
        $row4->layout = ['col-sm-3','col-sm-3','col-sm-3','col-sm-3'];

        $row5 = $this->form->addFields([$bcontainer_654e607d81be4],[$foto]);
        $row5->layout = [' col-sm-8',' col-sm-4'];

        $row6 = $this->form->addContent([new TFormSeparator("", '#333', '18', '#eee')]);
        $row7 = $this->form->addFields([new TLabel("Data de nascimento:", null, '12px', null, '100%'),$dt_nascimento_abertura],[new TLabel("CPF:", null, '12px', null, '100%'),$cpf_cnpj],[new TLabel("RG:", null, '12px', null, '100%'),$rg_ie],[new TLabel("Órgão emissor:", '#000000', '12px', null, '100%'),$orgao_emissor]);
        $row7->layout = [' col-sm-3',' col-sm-3',' col-sm-3',' col-sm-3'];

        $row8 = $this->form->addFields([new TLabel("Nacionalidade:", '#000000', '12px', null, '100%'),$nacionalidade_id],[new TLabel("Sexo:", '#000000', '12px', null, '100%'),$sexo_id],[new TLabel("Estado civil:", '#000000', '12px', null, '100%'),$estado_civil_id]);
        $row8->layout = ['col-sm-3','col-sm-3','col-sm-3'];

        $row9 = $this->form->addFields([new TLabel("Observação:", null, '12px', null, '100%'),$observacao]);
        ...
        $row10 = $this->detailFormPessoaEnderecoPessoa->addFields([new TLabel("CEP:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_cep,$button_buscar_pessoa_endereco_pessoa,$pessoa_endereco_pessoa_id],[new TLabel("Cidade:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_cidade_id]);
        $row10->layout = [' col-sm-4',' col-sm-8'];

        $row11 = $this->detailFormPessoaEnderecoPessoa->addFields([new TLabel("Bairro:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_bairro],[new TLabel("Rua:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_rua]);
        $row11->layout = [' col-sm-4',' col-sm-8'];

        $row12 = $this->detailFormPessoaEnderecoPessoa->addFields([new TLabel("Número:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_numero],[new TLabel("Complemento:", null, '12px', null, '100%'),$pessoa_endereco_pessoa_complemento],[new TLabel("Principal:", null, '12px', null, '100%'),$pessoa_endereco_pessoa_principal]);
        $row12->layout = ['col-sm-4',' col-sm-5',' col-sm-3'];
        ...
        });        $row15 = $this->form->addFields([$this->detailFormPessoaEnderecoPessoa]);
        $row15->layout = [' col-sm-12'];

        $this->form->appendPage("Especialidades");
        $row16 = $this->form->addFields([$this->fieldList_62d07079690f0]);
        $row16->layout = [' col-sm-12'];

        $this->form->appendPage("Informações de cadastro");
        $row17 = $this->form->addFields([new TLabel("Usuário do sistema:", '#FF0000', '12px', null, '100%'),$system_users_id]);
        $row17->layout = [' col-sm-12'];

        $row18 = $this->form->addFields([new TLabel("Criado em:", null, '12px', null, '100%'),$data_criacao],[new TLabel("Criado por:", null, '12px', null, '100%'),$criacao_user_name],[new TLabel("Atualizado em:", null, '12px', null, '100%'),$data_modificacao],[new TLabel("Atualizado por:", null, '12px', null, '100%'),$modificacao_user_name]);
        $row18->layout = ['col-sm-3','col-sm-3',' col-sm-3',' col-sm-3'];

        $row19 = $this->form->addFields([$chave_aasp],[],[],[]);
        $row19->layout = ['col-sm-3','col-sm-3',' col-sm-3',' col-sm-3'];
        ...
        $btn_onshow = $this->form->addAction("Sair", new TAction(['ProfissionalList', 'onShow']), 'fas:arrow-left #000000');
    ...
    public static function onLoadDocumentos($param = null) 
    {
        try 
        {
    ...
    public  function onChangeNome($param = null) 
    {
        try 
        {
            if(empty($param['id']) && !empty($param['nome'])){
                ...
                if($count>0){
                    ...
                    $pageParam['nova_pessoa_grupo'] = Grupo::PROFISSIONAL;
    ...
    public function onSave($param = null) 
    {
        try
        {
            ...
            if($data->tipo_profissional_id == TipoProfissional::ADVOGADO){
                if(!$data->registro_profissional){
                    throw new Exception("O campo Registro profissional é obrigatório.");
                }
                if(!$data->orgao_registro_profissional){
                    throw new Exception("O campo Orgão emissor é obrigatório.");
                }
            }
            ...
            if($data->cpf_cnpj){
                ...
                if($countCPF_CNPJ>0 && $data->tipo_pessoa_id==TipoPessoa::FISICA){
                    $verificarGrupoAtual = PessoaGrupo::where('grupo_id','=',Grupo::PROFISSIONAL)
                                                    ->where('pessoa_id','=',$searchCPF_CNPJ->id)
                                                    ->count();
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    if($verificarGrupoAtual>0){
                        new TQuestion("Este CPF já está cadastrado como profissional. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }else{
                        new TQuestion("Este CPF já está cadastrado. Deseja adicionar como profissional?", new TAction([__CLASS__, 'onAdicionarGrupoProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }
                    ...
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    if($verificarGrupoAtual>0){
                        new TQuestion("Este CNPJ já está cadastrado como profissional. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }else{
                        new TQuestion("Este CNPJ já está cadastrado. Deseja adicionar como profissional?", new TAction([__CLASS__, 'onAdicionarGrupoProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }
                }
            }
            ...
            if($data->rg_ie){
                ...
                if($countCPF_CNPJ>0 && $data->tipo_pessoa_id==TipoPessoa::FISICA){
                    ...
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    if($verificarGrupoAtual>0){
                        new TQuestion("Este RG já está cadastrado como profissional. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }else{
                        new TQuestion("Este RG já está cadastrado. Deseja adicionar como profissional?", new TAction([__CLASS__, 'onAdicionarGrupoProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }
                }elseif($countCPF_CNPJ>0 && $data->tipo_pessoa_id==TipoPessoa::JURIDICA){
                    $verificarGrupoAtual = PessoaGrupo::where('grupo_id','=',Grupo::PROFISSIONAL)
                    ...
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    if($verificarGrupoAtual>0){
                        new TQuestion("Esta Inscrição Estadual já está cadastrada como profissional. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }else{
                        new TQuestion("Esta Inscrição Estadual já está cadastrada. Deseja adicionar como profissional?", new TAction([__CLASS__, 'onAdicionarGrupoProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }
                }
            }
            ...
            if($data->registro_profissional && $data->orgao_registro_profissional){
                $countCPF_CNPJ = Pessoa::where('registro_profissional', '=', $data->registro_profissional)
                                        ->where('orgao_registro_profissional','=',$data->orgao_registro_profissional)
                                        ->where('id','!=',(int) $idAux)
                                        ->count();
                $searchCPF_CNPJ = Pessoa::where('registro_profissional', '=', $data->registro_profissional)
                                        ->where('orgao_registro_profissional','=',$data->orgao_registro_profissional)
                                        ->where('id','!=',(int) $idAux)
                                        ->first();
                if($countCPF_CNPJ>0){
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    new TQuestion("Este registro profissional já está cadastrado. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarProfissional'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                    exit;
                }
            }
...
//</generatedAutoCode>
    ...
    public function onDelete($param = null) 
    {
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                if ($object->loadComposite('Agenda', 'profissional_id', $object->id)) {
                    throw new Exception("Esse profissional possui uma agenda e não pode ser removido!");
                }

                if ($object->loadComposite('Conta', 'pessoa_id', $object->id)) {
                    throw new Exception("Esse profissional possui contas vinculadas e não pode ser removido!");
                }
                ...
                TApplication::loadPage('ProfissionalList', 'onShow');
    ...
    public static function onAdicionarGrupoProfissional($param = null) 
    {
        try 
        {
            TTransaction::open(self::$database); 
            ...
            $objeto->grupo_id = Grupo::PROFISSIONAL;
            $objeto->store();
            TApplication::loadPage('ProfissionalList', 'onShow');
            TTransaction::close();
        }
    ...
    public static function onEditarProfissional($param = null) 
    {
        try 
        {
