...
class FornecedorList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $globfiltfor = new TEntry('globfiltfor');

        $nome_col->exitOnEnter();
        $cpf_cnpj_col->exitOnEnter();
        $email_col->exitOnEnter();

        $globfiltfor->setEnterAction(new TAction([$this, 'onGlobalSearch']));
        ...
        $globfiltfor->setInnerIcon(new TImage('fas:search #9E9E9E'), 'left');
        $tipo_pessoa_id->enableSearch();
        $tipo_pessoa_col->enableSearch();

        $nome->forceUpperCase();
        $nome_col->forceUpperCase();
        $globfiltfor->forceUpperCase();
        ...
        $this->datagrid_form->addField($globfiltfor);
        ...
        $action_onEdit = new TDataGridAction(array('FornecedorForm', 'onEdit'));
        ...
        $panel = new TPanelGroup("Listagem de fornecedores");
        ...
        $button_buscar->setAction(new TAction(['FornecedorList', 'onGlobalSearch']), "Buscar");
        ...
        $button_cadastrar->setAction(new TAction(['FornecedorForm', 'onShow']), "Cadastrar");
        ...
        $button_filtros->setAction(new TAction(['FornecedorList', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_atualizar->setAction(new TAction(['FornecedorList', 'onRefresh']), "Atualizar");
        ...
        $button_limpar_filtros->setAction(new TAction(['FornecedorList', 'onClearFilters']), "Limpar filtros");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['FornecedorList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:table #00b894' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['FornecedorList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['FornecedorList', 'onExportXls']), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        ...
        $head_right_actions->add($globfiltfor);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Pessoas","Fornecedores"]));
        }
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            ...
            $page->setProperty('page-name', 'FornecedorListSearch');
            $page->setProperty('page_name', 'FornecedorListSearch');
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->globfiltfor) AND ( (is_scalar($data->globfiltfor) AND $data->globfiltfor !== '') OR (is_array($data->globfiltfor) AND (!empty($data->globfiltfor)) )) )
        {
            $globalCriteria = new TCriteria();

            $globalCriteria->add(new TFilter('cpf_cnpj', 'ilike', "%{$data->globfiltfor}%"), ' OR ');

            $globalCriteria->add(new TFilter('unaccent(nome)', 'ilike', "%{$data->globfiltfor}%"), ' OR ');

            $globalCriteria->add(new TFilter('telefone', 'ilike', "%{$data->globfiltfor}%"), ' OR ');
        ...
        if(!empty($data->globfiltfor)){
            TScript::create('$("[name=globfiltfor]").focus().each(function() {
