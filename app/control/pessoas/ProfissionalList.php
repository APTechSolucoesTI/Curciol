...
class ProfissionalList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $registro_profissional = new TEntry('registro_profissional');
        $orgao_registro_profissional = new TEntry('orgao_registro_profissional');
        $email_col = new TEntry('email_col');
        $globfil = new TEntry('globfil');

        $nome_col->exitOnEnter();
        $cpf_cnpj_col->exitOnEnter();
        $registro_profissional->exitOnEnter();
        $orgao_registro_profissional->exitOnEnter();
        $email_col->exitOnEnter();

        $globfil->setEnterAction(new TAction([$this, 'onGlobalSearch']));

        $nome_col->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1']));
        $cpf_cnpj_col->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1']));
        $registro_profissional->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1']));
        $orgao_registro_profissional->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1']));
        ...
        $globfil->setInnerIcon(new TImage('fas:search #9E9E9E'), 'left');
        $nome->forceUpperCase();
        $globfil->forceUpperCase();
        $nome_col->forceUpperCase();
        $orgao_registro_profissional->forceUpperCase();
        ...
        $row1 = $this->form->addFields([new TLabel("Nome:", null, '14px', null, '100%'),$nome],[new TLabel("Data nascimento:", null, '14px', null, '100%'),$dt_nascimento_abertura]);
        $row1->layout = [' col-sm-8',' col-sm-4'];

        $row2 = $this->form->addFields([new TLabel("Documento:", null, '14px', null),$cpf_cnpj],[new TLabel("Telefone:", null, '14px', null, '100%'),$telefone],[new TLabel("Email:", null, '14px', null),$email]);
        ...
        $this->datagrid_form->addField($globfil);
        $this->datagrid = new BootstrapDatagridWrapper($this->datagrid);
        $this->filter_criteria = new TCriteria;

        $filterVar = Grupo::PROFISSIONAL;
        $this->filter_criteria->add(new TFilter('id', 'in', "(SELECT pessoa_id FROM pessoa_grupo WHERE grupo_id = '{$filterVar}')"));
        ...
        $column_registro_profissional_transformed = new TDataGridColumn('registro_profissional', "Registro profissional", 'left');
        $column_orgao_registro_profissional = new TDataGridColumn('orgao_registro_profissional', "Orgão emissor", 'left');
        ...
        $column_registro_profissional_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
        ...
        $order_registro_profissional_transformed = new TAction(array($this, 'onReload'));
        $order_registro_profissional_transformed->setParameter('order', 'registro_profissional');
        $column_registro_profissional_transformed->setAction($order_registro_profissional_transformed);
        $order_orgao_registro_profissional = new TAction(array($this, 'onReload'));
        $order_orgao_registro_profissional->setParameter('order', 'orgao_registro_profissional');
        $column_orgao_registro_profissional->setAction($order_orgao_registro_profissional);
        ...
        $this->datagrid->addColumn($column_registro_profissional_transformed);
        $this->datagrid->addColumn($column_orgao_registro_profissional);
        ...
        $td_registro_profissional = TElement::tag('td', $registro_profissional);
        $tr->add($td_registro_profissional);
        $td_orgao_registro_profissional = TElement::tag('td', $orgao_registro_profissional);
        $tr->add($td_orgao_registro_profissional);
        ...
        $this->datagrid_form->addField($registro_profissional);
        $this->datagrid_form->addField($orgao_registro_profissional);
        ...
        $panel = new TPanelGroup("Listagem de profissionais");
        ...
        $button_buscar->setAction(new TAction(['ProfissionalList', 'onGlobalSearch']), "Buscar");
        ...
        $button_cadastrar->setAction(new TAction(['ProfissionalForm', 'onShow']), "Cadastrar");
        ...
        $button_filtros->setAction(new TAction(['ProfissionalList', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_atualizar->setAction(new TAction(['ProfissionalList', 'onRefresh']), "Atualizar");
        ...
        $button_limpar_filtros->setAction(new TAction(['ProfissionalList', 'onClearFilters']), "Limpar filtros");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['ProfissionalList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:table #00b894' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['ProfissionalList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['ProfissionalList', 'onExportXls']), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        ...
        $head_right_actions->add($globfil);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Pessoas","Profissionais"]));
        }
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            $object = new stdClass();
            $object->nome = null;
            $object->dt_nascimento_abertura = null;
            ...
            $page->setProperty('page-name', 'ProfissionalListSearch');
            $page->setProperty('page_name', 'ProfissionalListSearch');
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->registro_profissional) AND ( (is_scalar($data->registro_profissional) AND $data->registro_profissional !== '') OR (is_array($data->registro_profissional) AND (!empty($data->registro_profissional)) )) )
        {

            $filters[] = new TFilter('registro_profissional', 'like', "%{$data->registro_profissional}%");// create the filter 
        }

        if (isset($data->orgao_registro_profissional) AND ( (is_scalar($data->orgao_registro_profissional) AND $data->orgao_registro_profissional !== '') OR (is_array($data->orgao_registro_profissional) AND (!empty($data->orgao_registro_profissional)) )) )
        {

            $filters[] = new TFilter('orgao_registro_profissional', 'like', "%{$data->orgao_registro_profissional}%");// create the filter 
        }
        ...
        if (isset($data->globfil) AND ( (is_scalar($data->globfil) AND $data->globfil !== '') OR (is_array($data->globfil) AND (!empty($data->globfil)) )) )
        {
            $globalCriteria = new TCriteria();

            $globalCriteria->add(new TFilter('cpf_cnpj', 'ilike', "%{$data->globfil}%"), ' OR ');

            $globalCriteria->add(new TFilter('unaccent(nome)', 'ilike', "%{$data->globfil}%"), ' OR ');

            $globalCriteria->add(new TFilter('telefone', 'ilike', "%{$data->globfil}%"), ' OR ');
        ...
        if(!empty($data->globfil)){
            TScript::create('$("[name=globfil]").focus().each(function() {
