...
class ParceirosHeaderList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'formList_Parceiro';
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Listagem de parceiros");
        $this->limit = 20;

        $criteria_tipo_pessoa_id = new TCriteria();
        $criteria_tipo_pessoa_col = new TCriteria();
        ...
        $tipo_pessoa_col = new TDBCombo('tipo_pessoa_col', 'escritorio', 'TipoPessoa', 'id', '{nome}','nome asc' , $criteria_tipo_pessoa_col );
        $globfiltpar = new TEntry('globfiltpar');

        $nome_col->exitOnEnter();
        $cpf_cnpj_col->exitOnEnter();
        $email_col->exitOnEnter();

        $globfiltpar->setEnterAction(new TAction([$this, 'onGlobalSearch']));
        ...
        $tipo_pessoa_col->setChangeAction(new TAction([$this, 'onSearch'], ['static'=>'1']));

        $tipo_pessoa_id->setValue(TipoPessoa::FISICA);
        $email->forceLowerCase();
        $globfiltpar->setInnerIcon(new TImage('fas:search #9E9E9E'), 'left');
        $tipo_pessoa_id->enableSearch();
        $tipo_pessoa_col->enableSearch();

        $nome->forceUpperCase();
        $nome_col->forceUpperCase();
        $globfiltpar->forceUpperCase();
        ...
        $globfiltpar->setSize(200);
        $email_col->setSize('100%');
        $cpf_cnpj_col->setSize('100%');
        $tipo_pessoa_id->setSize('100%');
        $tipo_pessoa_col->setSize('100%');

        //<onBeforeAddFieldsToForm>

        //</onBeforeAddFieldsToForm>
        $row1 = $this->form->addFields([new TLabel("Tipo de pessoa:", null, '14px', null),$tipo_pessoa_id],[new TLabel("Nome:", null, '14px', null, '100%'),$nome]);
        $row1->layout = [' col-sm-5',' col-sm-7'];

        $row2 = $this->form->addFields([new TLabel("Documento:", null, '14px', null, '100%'),$cpf_cnpj],[new TLabel("Telefone:", null, '14px', null, '100%'),$telefone],[new TLabel("Email:", null, '14px', null),$email]);
        ...
        $this->datagrid_form->addField($globfiltpar);
        $this->datagrid = new BootstrapDatagridWrapper($this->datagrid);
        $this->filter_criteria = new TCriteria;

        $filterVar = Grupo::PARCEIRO;
        ...
        $action_onEdit = new TDataGridAction(array('ParceiroForm', 'onEdit'));
        ...
        $tr->add($td_email_col);
        $td_tipo_pessoa_col = TElement::tag('td', $tipo_pessoa_col);
        $tr->add($td_tipo_pessoa_col);
        $tr->add(TElement::tag('td', ''));

        $this->datagrid_form->addField($nome_col);
        $this->datagrid_form->addField($cpf_cnpj_col);
        $this->datagrid_form->addField($email_col);
        $this->datagrid_form->addField($tipo_pessoa_col);
        ...
        $panel = new TPanelGroup("Listagem de parceiros");
        ...
        $button_buscar->setAction(new TAction(['ParceirosHeaderList', 'onGlobalSearch']), "Buscar");
        ...
        $button_cadastrar->setAction(new TAction(['ParceiroForm', 'onShow']), "Cadastrar");
        ...
        $button_filtros->setAction(new TAction(['ParceirosHeaderList', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_atualizar->setAction(new TAction(['ParceirosHeaderList', 'onRefresh']), "Atualizar");
        ...
        $button_limpar_filtros->setAction(new TAction(['ParceirosHeaderList', 'onClearFilters']), "Limpar filtros");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['ParceirosHeaderList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:table #00b894' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['ParceirosHeaderList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['ParceirosHeaderList', 'onExportXls']), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        ...
        $head_right_actions->add($globfiltpar);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Pessoas","Parceiros"]));
        }
...
//<generated-DatagridHeaderAction-onGlobalSearch>
    public function onGlobalSearch($param = null) 
    {

        //</onGlobalSearchAutoCode>
    //<generatedAutoCode>
...
//<generatedAutoCode>
            ...
            $page->setProperty('page-name', 'ParceirosHeaderListSearch');
            $page->setProperty('page_name', 'ParceirosHeaderListSearch');
...
//</generated-DatagridHeaderAction-onExportXls>
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->tipo_pessoa_col) AND ( (is_scalar($data->tipo_pessoa_col) AND $data->tipo_pessoa_col !== '') OR (is_array($data->tipo_pessoa_col) AND (!empty($data->tipo_pessoa_col)) )) )
        {

            $filters[] = new TFilter('tipo_pessoa_id', '=', $data->tipo_pessoa_col);// create the filter 
        }

        if (isset($data->globfiltpar) AND ( (is_scalar($data->globfiltpar) AND $data->globfiltpar !== '') OR (is_array($data->globfiltpar) AND (!empty($data->globfiltpar)) )) )
        {
            $globalCriteria = new TCriteria();

            $globalCriteria->add(new TFilter('cpf_cnpj', 'ilike', "%{$data->globfiltpar}%"), ' OR ');

            $globalCriteria->add(new TFilter('unaccent(nome)', 'ilike', "%{$data->globfiltpar}%"), ' OR ');

            $globalCriteria->add(new TFilter('telefone', 'ilike', "%{$data->globfiltpar}%"), ' OR ');
        ...
        if(!empty($data->globfiltpar)){
            TScript::create('$("[name=globfiltpar]").focus().each(function() {
