...
class ContraparteForm extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $this->form->setFormTitle("Cadastro de parte contraria");
        ...
        $criteria_classificacoes_contraparte_id = new TCriteria();
        ...
        $classificacoes_contraparte_id = new TDBCheckGroup('classificacoes_contraparte_id', 'escritorio', 'ClassificacoesContraparteDados', 'id', '{nome}','nome asc' , $criteria_classificacoes_contraparte_id );
        ...
        $classificacoes_contraparte_id->setLayout('horizontal');
        $classificacoes_contraparte_id->setUseButton();
        ...
        $nome->setTip("Nome da parte contraria");
        ...
        $classificacoes_contraparte_id->setSize(150);
        ...
        $bcontainer_654e625781bfb = new BootstrapFormBuilder('bcontainer_654e625781bfb');
        $this->bcontainer_654e625781bfb = $bcontainer_654e625781bfb;
        $bcontainer_654e625781bfb->setProperty('style', 'border:none; box-shadow:none;');
        $row2 = $bcontainer_654e625781bfb->addFields([new TLabel("Tipo de pessoa:", '#FF0000', '12px', null, '100%'),$tipo_pessoa_id,$btnBuscarCNPJ],[new TLabel("Nome:", '#ff0000', '12px', null, '100%'),$nome,$btnVerificarNome]);
        $row2->layout = ['col-sm-6',' col-sm-6'];

        $row3 = $bcontainer_654e625781bfb->addFields([new TLabel("Telefone:", null, '12px', null, '100%'),$telefone],[new TLabel("Email:", null, '12px', null, '100%'),$email]);
        $row3->layout = ['col-sm-6','col-sm-6'];

        $row4 = $this->form->addFields([$bcontainer_654e625781bfb],[$foto]);
        ...
        $row9 = $this->form->addFields([new TLabel("Profissão:", '#000000', '11px', null, '100%'),$profissao],[new TLabel("NIT:", '#000000', '12px', null, '100%'),$nit],[new TLabel("CTPS:", '#000000', '12px', null, '100%'),$ctps]);
        ...
        $row11 = $this->form->addFields([new TLabel("Selecione as classificações da Parte Contrária:", null, '14px', null),$classificacoes_contraparte_id]);
        $row11->layout = [' col-sm-6'];

        $row12 = $this->form->addFields([new TLabel("Observação:", null, '12px', null, '100%'),$observacao]);
        ...
        $row13 = $this->detailFormPessoaEnderecoPessoa->addFields([new TLabel("CEP:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_cep,$button_buscar_pessoa_endereco_pessoa],[new TLabel("Cidade:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_cidade_id,$pessoa_endereco_pessoa_id]);
        $row13->layout = [' col-sm-4',' col-sm-8'];

        $row14 = $this->detailFormPessoaEnderecoPessoa->addFields([new TLabel("Bairro:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_bairro],[new TLabel("Rua:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_rua]);
        $row14->layout = [' col-sm-4',' col-sm-8'];

        $row15 = $this->detailFormPessoaEnderecoPessoa->addFields([new TLabel("Número:", '#ff0000', '12px', null, '100%'),$pessoa_endereco_pessoa_numero],[new TLabel("Complemento:", null, '12px', null, '100%'),$pessoa_endereco_pessoa_complemento],[new TLabel("Principal:", null, '12px', null, '100%'),$pessoa_endereco_pessoa_principal]);
        $row15->layout = ['col-sm-4',' col-sm-5',' col-sm-3'];

        $row16 = $this->detailFormPessoaEnderecoPessoa->addFields([$button_adicionar_pessoa_endereco_pessoa]);
        $row16->layout = [' col-sm-12'];

        $row17 = $this->detailFormPessoaEnderecoPessoa->addFields([new THidden('pessoa_endereco_pessoa__row__id')]);
        ...
        $action_onEditDetailPessoaEndereco = new TDataGridAction(array('ContraparteForm', 'onEditDetailPessoaEndereco'));
        ...
        $action_onDeleteDetailPessoaEndereco = new TDataGridAction(array('ContraparteForm', 'onDeleteDetailPessoaEndereco'));
        ...
        $row19 = $this->detailFormPessoaContatoPessoa->addFields([new TLabel("Descrição:", '#ff0000', '12px', null, '100%'),$pessoa_contato_pessoa_descricao,$pessoa_contato_pessoa_id]);
        $row19->layout = [' col-sm-12'];

        $row20 = $this->detailFormPessoaContatoPessoa->addFields([new TLabel("Telefone:", null, '12px', null, '100%'),$pessoa_contato_pessoa_telefone],[new TLabel("Email:", null, '12px', null, '100%'),$pessoa_contato_pessoa_email]);
        $row20->layout = ['col-sm-6','col-sm-6'];

        $row21 = $this->detailFormPessoaContatoPessoa->addFields([$button_adicionar_pessoa_contato_pessoa]);
        $row21->layout = [' col-sm-12'];

        $row22 = $this->detailFormPessoaContatoPessoa->addFields([new THidden('pessoa_contato_pessoa__row__id')]);
        ...
        $action_onEditDetailPessoaContato = new TDataGridAction(array('ContraparteForm', 'onEditDetailPessoaContato'));
        ...
        $action_onDeleteDetailPessoaContato = new TDataGridAction(array('ContraparteForm', 'onDeleteDetailPessoaContato'));
        ...
        $row23 = $this->form->addFields([$this->detailFormPessoaContatoPessoa]);
        ...
        $btn_onshow = $this->form->addAction("Sair", new TAction(['ContraparteList', 'onShow']), 'fas:arrow-left #000000');
        ...
        $style = new TStyle('right-panel > .container-part[page-name=ContraparteForm]');
        ...
        try 
        {
            if(empty($param['id']) && !empty($param['nome'])){
                ...
                if($count>0){
                    ...
                    $pageParam['nova_pessoa_grupo'] = Grupo::CONTRAPARTE;
        ...
        try
        {
            ...
            if($data->cpf_cnpj){
                ...
                if($countCPF_CNPJ>0 && $data->tipo_pessoa_id==TipoPessoa::FISICA){
                    $verificarGrupoAtual = PessoaGrupo::where('grupo_id','=',Grupo::CONTRAPARTE)
                                                    ->where('pessoa_id','=',$searchCPF_CNPJ->id)
                                                    ->count();
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    if($verificarGrupoAtual>0){
                        new TQuestion("Este CPF já está cadastrado como parte contraria. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarContraparte'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }else{
                        new TQuestion("Este CPF já está cadastrado. Deseja adicionar como parte contraria?", new TAction([__CLASS__, 'onAdicionarGrupoContraparte'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }
                    ...
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    if($verificarGrupoAtual>0){
                        new TQuestion("Este CNPJ já está cadastrado como parte contraria. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarContraparte'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }else{
                        new TQuestion("Este CNPJ já está cadastrado. Deseja adicionar como parte contraria?", new TAction([__CLASS__, 'onAdicionarGrupoContraparte'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }
                }
            }
            ...
            if($data->rg_ie){
                ...
                if($countCPF_CNPJ>0 && $data->tipo_pessoa_id==TipoPessoa::FISICA){
                    ...
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    if($verificarGrupoAtual>0){
                        new TQuestion("Este RG já está cadastrado como parte contraria. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarContraparte'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }else{
                        new TQuestion("Este RG já está cadastrado. Deseja adicionar como parte contraria?", new TAction([__CLASS__, 'onAdicionarGrupoContraparte'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }
                }elseif($countCPF_CNPJ>0 && $data->tipo_pessoa_id==TipoPessoa::JURIDICA){
                    $verificarGrupoAtual = PessoaGrupo::where('grupo_id','=',Grupo::CONTRAPARTE)
                    ...
                    $pageParam['pessoa_id'] = $searchCPF_CNPJ->id;
                    if($verificarGrupoAtual>0){
                        new TQuestion("Esta Inscrição Estadual já está cadastrada como parte contraria. Deseja editar o cadastro existente?", new TAction([__CLASS__, 'onEditarContraparte'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }else{
                        new TQuestion("Esta Inscrição Estadual já está cadastrada. Deseja adicionar como parte contraria?", new TAction([__CLASS__, 'onAdicionarGrupoContraparte'], $pageParam), new TAction([__CLASS__, 'btnNaoTQuestion'], $pageParam));
                        exit;
                    }
                }
            }
            ...
            $repository = ClassificacoesContraparte::where('pessoa_id', '=', $object->id);
            $repository->delete(); 

            if ($data->classificacoes_contraparte_id) 
            {
                foreach ($data->classificacoes_contraparte_id as $classificacoes_contraparte_id_value) 
                {
                    $classificacoes_contraparte = new ClassificacoesContraparte;

                    $classificacoes_contraparte->classificacoes_contraparte_dados_id = $classificacoes_contraparte_id_value;
                    $classificacoes_contraparte->pessoa_id = $object->id;
                    $classificacoes_contraparte->store();
                }
            }
...
//</generatedAutoCode>
            ...
            if(!$data->id)
            {
                ...
                $pessoaGrupo->grupo_id = Grupo::CONTRAPARTE;
                $pessoaGrupo->store();    
            }
            ...
            TApplication::loadPage('ContraparteList', 'onShow', $loadPageParam); 
        ...
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                if ($object->loadComposite('PessoaRepresentantesLegais', 'pessoa_juridica_id', $object->id)) {
                    throw new Exception("Essa parte contraria possui representantes legais e não pode ser removido!");
                }

                if($qtdeGrupos>1){
                    $grupoExcluir = PessoaGrupo::where('pessoa_id', '=', $object->id)
                                                ->where('grupo_id', '=', Grupo::CONTRAPARTE);
                    $grupoExcluir->delete();
                }else{
...
//</generatedAutoCode>
    ...
    public static function onAdicionarGrupoContraparte($param = null) 
    {
        try 
        {
            TTransaction::open(self::$database); 
            ...
            $objeto->grupo_id = Grupo::CONTRAPARTE;
            $objeto->store();
            TApplication::loadPage('ContraparteList', 'onShow');
            TTransaction::close();
        }
        catch (Exception $e) 
        {
    ...
    public static function onEditarContraparte($param = null) 
    {
        try 
        {
