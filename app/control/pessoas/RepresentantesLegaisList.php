...
class RepresentantesLegaisList extends TPage
{
    private $form; // form
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Listagem de representantes legais");
        $this->limit = 20;
        ...
        $globalfilt = new TEntry('globalfilt');
        ...
        $globalfilt->setEnterAction(new TAction([$this, 'onGlobalSearch']));
        ...
        $globalfilt->setInnerIcon(new TImage('fas:search #9E9E9E'), 'left');
        $nome->forceUpperCase();
        $nome_col->forceUpperCase();
        $globalfilt->forceUpperCase();

        $nome->setSize('100%');
        $email->setSize('100%');
        $globalfilt->setSize(200);
        ...
        $this->datagrid_form->addField($globalfilt);
        ...
        $column_cpf_cnpj_transformed = new TDataGridColumn('cpf_cnpj', "CPF", 'left');
        ...
        $action_onEdit = new TDataGridAction(array('RepresentanteLegalForm', 'onEdit'));
        ...
        $panel = new TPanelGroup("Listagem de representantes legais");
        ...
        $button_buscar->setAction(new TAction(['RepresentantesLegaisList', 'onGlobalSearch']), "Buscar");
        ...
        $button_cadastrar->setAction(new TAction(['RepresentanteLegalForm', 'onShow']), "Cadastrar");
        ...
        $button_filtros->setAction(new TAction(['RepresentantesLegaisList', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_atualizar->setAction(new TAction(['RepresentantesLegaisList', 'onRefresh']), "Atualizar");
        ...
        $button_limpar_filtros->setAction(new TAction(['RepresentantesLegaisList', 'onClearFilters']), "Limpar filtros");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['RepresentantesLegaisList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:table #00b894' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['RepresentantesLegaisList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['RepresentantesLegaisList', 'onExportXls']), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        ...
        $head_right_actions->add($globalfilt);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Pessoas","Representantes Legais"]));
        }
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            ...
            */
            ...
            $page->setProperty('page-name', 'RepresentantesLegaisListSearch');
            $page->setProperty('page_name', 'RepresentantesLegaisListSearch');
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->globalfilt) AND ( (is_scalar($data->globalfilt) AND $data->globalfilt !== '') OR (is_array($data->globalfilt) AND (!empty($data->globalfilt)) )) )
        {
            $globalCriteria = new TCriteria();

            $globalCriteria->add(new TFilter('cpf_cnpj', 'ilike', "%{$data->globalfilt}%"), ' OR ');

            $globalCriteria->add(new TFilter('unaccent(nome)', 'ilike', "%{$data->globalfilt}%"), ' OR ');

            $globalCriteria->add(new TFilter('telefone', 'ilike', "%{$data->globalfilt}%"), ' OR ');
        ...
        if(!empty($data->globalfilt)){
            TScript::create('$("[name=globalfilt]").focus().each(function() {
