...
class RequisicaoPagamentoVisualizacao extends TWindow
{
    protected $form;

    private static $database = 'escritorio';
    private static $formName = 'form_RequisicaoPagamentoVisualizacaoWindow';
    ...
    public function __construct($param = null)
    {
        ...
        $modoEmbutido = !empty($param['modo_embutido']);

        if (!$modoEmbutido) {
            parent::setTitle('Editar requisição de pagamento');
            parent::setSize(0.78, 0.92);
        }

        $this->aplicarCss();

        if (!empty($param['processo_id'])) {
            $this->montarPorProcesso($param['processo_id'], $modoEmbutido);
            ...
            $this->montarVisualizacao(
                ...
                $modoEmbutido,
                null,
                $param['requisicao_pagamento_cliente_id'] ?? null
            );
        }
    }
    ...
    private static function criarAvisoSemRequisicao($processoId, $processo = null)
    {
        ...
        $div->add("
            ...
            <div class='req-empty-text'>
                O processo <strong>{$numero}</strong> ainda não possui uma requisição de pagamento vinculada para este cliente.
            </div>
        ");
    ...
    private function montarPorProcesso($processoId, $modoEmbutido = false)
    {
        try {
            ...
            $dados = $this->buscarRequisicaoPorProcesso($conn, $processoId);
           ...
           $this->montarVisualizacao(
                ...
                $modoEmbutido,
                $processoId,
                $dados->requisicao_pagamento_cliente_id ?? null
            );
        }
        catch (Exception $e) {
    ...
    private function montarVisualizacao($requisicao_id, $pessoaSelecionadaId = null, $modoEmbutido = false, $processoOrigemId = null, $rpcSelecionadoId = null)
    {
        try {
            TTransaction::open(self::$database);
            ...
            foreach ($clientes as $cliente) {
              ...
              $clienteSelecionado = false;
                ...
                if (!empty($rpcSelecionadoId) && (int) $rpcId === (int) $rpcSelecionadoId) {
                    $clienteSelecionado = true;
                }

                if (
                    empty($rpcSelecionadoId)
                    && !empty($pessoaSelecionadaId)
                    && (int) $cliente->cliente_id === (int) $pessoaSelecionadaId
                ) {
                    $clienteSelecionado = true;
                }
               ...
               if ($ehMle) {
                ...
                if ($ehMle) {
                    $data->{'data_requerimento_' . $rpcId} = self::formatarDataBR(
                        $cliente->data_requerimento ?: $cliente->etapa3_data_pedido_mle
                    );

                    $data->{'data_base_' . $rpcId} = self::formatarDataBR(
                        $cliente->data_base ?: $cliente->etapa3_data_deposito
                    );

                    $data->{'valor_' . $rpcId} = $cliente->valor ?: $cliente->etapa3_valor_mle;
            ...
            $this->form->setCurrentPage($paginaSelecionada);

            $this->form->addAction(
                'Salvar',
                new TAction([$this, 'onSave'], ['key' => $requisicao_id]),
                'far:save #ffffff'
           ...
           TScript::create("
            setTimeout(function() {
                ...
                var janela = form.closest('.ui-dialog, .twindow, .modal, .window');
                ...
                $('.ui-widget-overlay:last, .modal-backdrop:last, .window_modal:last').appendTo('body').css({
                    'z-index': '19990'
                });

                var paginaSelecionada = " . (int) $paginaSelecionada . ";
                var aba = form.find('.nav-tabs li').eq(paginaSelecionada).find('a').first();

                if (!aba.length) {
                    aba = form.find('.nav-tabs li:first a').first();
                }
                ...
                if (aba.length) {
                    ...
                    if (aba[0] && aba[0].scrollIntoView) {
                        aba[0].scrollIntoView({
                            behavior: 'smooth',
                            block: 'nearest',
                            inline: 'center'
                        });
                ...
                form.find('[name^=\"etapa3_possui_saldo_\"]').each(function() {
                    $(this).trigger('change');
                });
            }, 400);

            $(document).off('select2:open.reqwindow');

            $(document).on('select2:open.reqwindow', function() {
                setTimeout(function() {
        ...
        try {
            ...
            foreach ($clientes as $cliente) {
               ...
               $sqlCliente = "
                ...
                $this->salvarEtapa3($conn, $param, $rpcId, $ehMle, $requisicao->processo_id);
            }

            TTransaction::close();

            new TMessage('info', 'Requisição de pagamento salva com sucesso.');
    ...
    private function salvarEtapa2($conn, $param, $rpcId)
    {
        ...
        $protocolo = self::dataParaBanco(self::valorParam($param, 'etapa2_protocolo_depre_entidade_devedora_' . $rpcId));
        $numero = self::valorParam($param, 'etapa2_numero_depre_entidade_devedora_' . $rpcId);
  ...
  private static function criarBotaoVerProcesso($processoId)
    {
        ...
        $url = 'engine.php?' . http_build_query([
        ...
        return "
            ...
            onclick=\"
                ...
                setTimeout(function() {
                    $('#adianti_right_panel').css({
                        'z-index': '30050'
                    });

                    $('.right-panel, .adianti_right_panel, #adianti_right_panel').css({
                        'z-index': '30050'
                    });
                }, 300);
    ...
    private function criarScriptSaldoParcial($rpcId)
    {
        TScript::create("
            setTimeout(function() {
                ...
                function getColunaDoCampo{$rpcId}(nomeCampo) {
                    var campo = form.find('[name=\"' + nomeCampo + '\"]').last();
                ...
                function toggleSaldoParcial{$rpcId}() {
                    var campoPossuiSaldo = form.find('[name=\"etapa3_possui_saldo_{$rpcId}\"]').last();

                    if (!campoPossuiSaldo.length) {
                        return;
                    }

                    var valor = campoPossuiSaldo.val();
                    var mostrar = valor === 'S';
                    ...
                    campos.forEach(function(nomeCampo) {
                        var campo = form.find('[name=\"' + nomeCampo + '\"]').last();
                        var coluna = getColunaDoCampo{$rpcId}(nomeCampo);
                    ...
                    var ajuda = form.find('[id=\"req_partial_help_{$rpcId}\"]').last();
                ...
                $(document).off('change.saldo{$rpcId} select2:select.saldo{$rpcId}', '#" . self::$formName . " [name=\"etapa3_possui_saldo_{$rpcId}\"]');

                $(document).on('change.saldo{$rpcId} select2:select.saldo{$rpcId}', '#" . self::$formName . " [name=\"etapa3_possui_saldo_{$rpcId}\"]', function() {
                    toggleSaldoParcial{$rpcId}();
                });

                toggleSaldoParcial{$rpcId}();

                setTimeout(toggleSaldoParcial{$rpcId}, 500);
                setTimeout(toggleSaldoParcial{$rpcId}, 1000);
            }, 350);
        ");
    }
