...
class AndamentoSeekWindow extends TWindow
{
    private $form; // form
    ...
    private static $formName = 'form_AndamentoSeekWindow';
    ...
    public function __construct($param = null)
    {
        ...
        parent::setTitle("Vincular andamento a tarefa");
        ...
        $this->form->setFormTitle("Vincular andamento a tarefa");
        ...
        $processo_id->setFilterColumns(["numero_cnj_numero"]);
        $processo_id->disableIdSearch();
        $titulo->setMaxLength(255);
        $titulo->setSize('100%');
        $processo_id->setSize('100%');

        $row1 = $this->form->addFields([new TLabel("Processo:", null, '14px', null, '100%'),$processo_id],[new TLabel("Titulo:", null, '14px', null, '100%'),$titulo]);
        ...
        $column_processo_numero_cnj_numero = new TDataGridColumn('processo->numero_cnj_numero', "Processo", 'left');
        $column_titulo = new TDataGridColumn('titulo', "Titulo", 'left');
        $column_data_criacao = new TDataGridColumn('data_criacao', "Criado em", 'left');

        $this->datagrid->addColumn($column_processo_numero_cnj_numero);
        $this->datagrid->addColumn($column_titulo);
        $this->datagrid->addColumn($column_data_criacao);

        $action_onSelect = new TDataGridAction(array('AndamentoSeekWindow', 'onSelect'));
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->processo_id) AND ( (is_scalar($data->processo_id) AND $data->processo_id !== '') OR (is_array($data->processo_id) AND (!empty($data->processo_id)) )) )
        {

            $filters[] = new TFilter('processo_id', '=', $data->processo_id);// create the filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Andamento
            $repository = new TRepository(self::$activeRecord);
