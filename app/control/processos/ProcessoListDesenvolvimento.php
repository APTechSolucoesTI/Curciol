...
class ProcessoListDesenvolvimento extends TPage
{
    private $form; // form
    ...
    public function __construct($param = null)
    {
        ...
        $column_numero_cnj_numero = new TDataGridColumn('numero_cnj_numero', "Número", 'left' , '250px');
        $column_responsavel_nome = new TDataGridColumn('responsavel->nome', "Responsável", 'left' , '90px');
        $column_id_transformed1 = new TDataGridColumn('id', "Clientes", 'left' , '200px');
        $column_contrapartes_nome = new TDataGridColumn('contrapartes_nome', "Contrapartes", 'left' , '200px');
        $column_id_transformed2 = new TDataGridColumn('id', " ", 'left');
        ...
        $btnShowCurtainFilters->setAction(new TAction(['ProcessoListDesenvolvimento', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_limpar_filtros->setAction(new TAction(['ProcessoListDesenvolvimento', 'onClearFilters']), "Limpar filtros");
        ...
        $button_atualizar->setAction(new TAction(['ProcessoListDesenvolvimento', 'onRefresh']), "Atualizar");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['ProcessoListDesenvolvimento', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['ProcessoListDesenvolvimento', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['ProcessoListDesenvolvimento', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['ProcessoListDesenvolvimento', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Processos","Processos Parte Cliente"]));
        }
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            ...
            $page->setProperty('page-name', 'ProcessoListDesenvolvimentoSearch');
            $page->setProperty('page_name', 'ProcessoListDesenvolvimentoSearch');
            ...
            $style = new TStyle('right-panel > .container-part[page-name=ProcessoListDesenvolvimentoSearch]');
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->cliente_col) AND ( (is_scalar($data->cliente_col) AND $data->cliente_col !== '') OR (is_array($data->cliente_col) AND (!empty($data->cliente_col)) )) )
        {
            $filters[] = new TFilter('id', 'in', "(SELECT processo_id FROM contrato_processo WHERE contrato_id in (SELECT contrato_id FROM contrato_pessoa WHERE cliente_id = $data->cliente_col))");
        }
